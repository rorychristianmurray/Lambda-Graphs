...
s = Stack()
prev_room = 0
bfs_possible_directions_count = 0
...
while unexplored is True: # while curr room has unexplored paths
    ...
    print(f"prev_room 76 : {prev_room}")
    ...
    if room not in visited:
        ...
        if not traversalPath[-1]:
            ...
            print(f"last_dir 94 : {last_dir}")
            print(f"traversalPath 95 : {traversalPath}")
            last_dir = traversalPath[-1]
            print(f"last_dir 97 : {last_dir}")
            print(f"prev_room 98 : {prev_room}")
    ...
    print(f"\npossible_directions 161: {possible_directions}\n")

    # randomly choose one unexplored

    # global poss_dirs_count
    poss_dirs_count = 0
    ...
    print(f"\nposs_dirs_count : {poss_dirs_count}\n")
    ...
    if poss_dirs_count == 0:
        ...
        def bfs(starting_room):
            ...
            while q.size() > 0:
                ...
                curr_room = path[-1]
                print(f"curr_room in bfs : {curr_room}")

                if curr_room not in bfs_visited:
                    # declare directions
                    if curr_room not in visited:
                        print("hit oil")
                        print(f"bfs_visited : {bfs_visited}")
                        print("\ncheck\n")
                        check = bfs_visited[curr_room]
                        print(f"\ncheck : {check}\n")
                        return
                    directions = visited[curr_room]
                    print(f"bfs directions : {directions}")
                    
                    ## mark as visited
                    bfs_visited[curr_room] = path
                directions = visited[curr_room]
                for direction, next_room in directions.items():
                    ## if next_room = '?'
                    ## then curr_room is our
                    ## terminating node
                    if next_room == '?':
                        # we want to the path to curr_room
                        # which we get from bfs_visited[curr_room]
                        print(f"terminating room is {curr_room}")
                        print(f"bfs_visited[curr_room] {bfs_visited[curr_room]}")
                        path_to_target = bfs_visited[curr_room][1:]
                        print(f"path_to_target : {path_to_target}")

                        ## move to target
                        ## by following moves 
                        ## in order and turning to directions
                        ## pop off front
                        while len(path_to_target) > 0:
                            move_to_room = path_to_target.pop(0)
                            ## convert to direction
                            player_room = player.currentRoom.id
                            curr_dirs = visited[player_room]
                            print(f"\ncurr_dirs : {curr_dirs}\n")
                            ...
                            for k, v in curr_dirs.items():
                                ...
                                if v == move_to_room:
                                    ...
                                    print(f"traversalPath 251 : {traversalPath}")
                                    player.travel(move_dir)

                        ## reset possible_directions
                        ## and poss_dirs_count
                        curr_room_now = player.currentRoom.id
                        print(f"curr_room_now : {curr_room_now}")
                        poss_dir_now = visited[curr_room_now]
                        print(f"poss_dir_now : {poss_dir_now}")
                        # global poss_dirs_count
                        # global poss_dirs_count_now
                        # poss_dirs_count_now = 0

                        # poss_dirs_count = 0
                        global bfs_possible_directions_count
                        print(f"bfs_possible_directions_count : {bfs_possible_directions_count}")
                        for dir in poss_dir_now:
                            if poss_dir_now[dir] == '?':
                                ## add unexplored directions to the stack
                                # poss_dirs_count_now += 1
                                bfs_possible_directions_count +=1

                        # print(f"\nposs_dirs_count_now 267 : {poss_dirs_count_now}\n")
                        print(f"\nbfs_possible_directions_count 275 : {bfs_possible_directions_count}\n")

                        ## set unexplored to True
                        ## return out
                        global unexplored
                        unexplored = True
                        return
        ...
        # print(f"pass_bfs_room : {pass_bfs_room}")
        # print("outside hit bef")
        bfs(pass_bfs_room)
        # print(f"poss_dirs_count_now 292 : {poss_dirs_count_now}")
        print(f"bfs_possible_directions_count 293 : {bfs_possible_directions_count}")

        
    if bfs_possible_directions_count > 0:
        poss_dirs_count = bfs_possible_directions_count
        print(f"poss_dirs_count 300 : {poss_dirs_count}")
        bfs_possible_directions_count = 0
        player_room = player.currentRoom.id
        print(f"player_room : {player_room}")
        possible_directions = visited[player_room]  
    ...
    print(f"\npossible_directions 306 : {possible_directions}\n")

    ## convert to cardinal direction
    ## go the choose_dir'th direction available
    dir_array = []
    for dir, room in possible_directions.items():
        print(f"\ndir in possible_direction : {dir}\n")
        if room == '?':
    ...
    print(f"traversalPath 324 {traversalPath}")
